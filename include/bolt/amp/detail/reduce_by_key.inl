    ...
    const BinaryFunction& binary_op)
{
	...
	int numElements = static_cast<int>(keys_last - keys_first);
    if (numElements == 1)
        return 1;
    ...
    unsigned int sizeOfOut = amp::reduce_by_key( ctl, dvKeys.begin( ), dvKeys.end( ), dvValues.begin(), dvKOutput.begin( ),
