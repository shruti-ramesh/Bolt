...
aProfiler.setDataSize(numElements*sizeof(oType));
	...
	const BinaryFunction& binary_op)
	{
		...
		device_vector< iType, concurrency::array_view > dvInput( first, last, false, ctrl );
        device_vector< oType, concurrency::array_view > dvOutput( result, numElements, true, ctrl );
	    
        //Now call the actual AMP algorithm
        amp::scan( ctrl, dvInput.begin( ), dvInput.end( ), dvOutput.begin( ), init, inclusive, binary_op );
