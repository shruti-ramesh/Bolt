...
//#define _REDUCE_STEP_MIN(_LENGTH, _IDX, _W) \
//    if ((_IDX < _W) && ((_IDX + _W) < _LENGTH)) {\
//	  bool stat = binary_op(scratch[_IDX], scratch[_IDX + _W]); \
//      scratch[_IDX] = stat ? scratch[_IDX] : scratch[_IDX + _W] ;\
//      scratch_index[_IDX] = stat ? scratch_index[_IDX]:scratch_index[_IDX + _W];\
//    }\
//    t_idx.barrier.wait();
...
//#define _REDUCE_STEP_MAX(_LENGTH, _IDX, _W)\
//	if ((_IDX < _W) && ((_IDX + _W) < _LENGTH)) {\
//		bool stat = binary_op(scratch[_IDX + _W], scratch[_IDX]);\
//		scratch[_IDX] = stat ? scratch[_IDX] : scratch[_IDX + _W] ;\
//		scratch_index[_IDX] = stat ? scratch_index[_IDX]:scratch_index[_IDX + _W];\
//		}\
//   t_idx.barrier.wait();
    ...
    namespace amp {
        namespace detail {
                ...
                const char * min_max )
            {
				...
				try
                {
                                                   ...
                                                   ( concurrency::tiled_index<tileSize> t_idx ) restrict(amp)
                    {
                      ...
                      tile_static iType scratch [WAVEFRONT_SIZE*sizeof(iType)] ;
					  tile_static int scratch_index [WAVEFRONT_SIZE*sizeof(int)] ;
					...
					iType minele =  cpuPointerReduce[0];
					//iType minele =  *first + cpuPointerReduce[0];
					//iType minele_indx = cpuPointerReduce[0] ;
					int minele_indx = 0;
                    ...
                    {
                        bool stat = binary_op( minele, cpuPointerReduce[i]);
						minele = stat ? minele : cpuPointerReduce[i];
						//minele_indx =  stat ? minele_indx : cpuPointerReduce[i];
						minele_indx =  stat ? minele_indx : i;
				...
				try
                {
                    ...
                    for(int i = 0; i < numTailReduce; ++i)
                    {
                        bool stat = binary_op(cpuPointerReduce[i], minele);
						minele = stat ? minele : cpuPointerReduce[i];
						//minele_indx =  stat ? minele_indx : cpuPointerReduce[i];
						minele_indx =  stat ? minele_indx : i;
                    }
                ...
                const char * min_max)
            {
                ...
                default:
			      { 
					/*std::enable_if< !std::is_base_of<typename device_vector<typename std::
                    iterator_traits<ForwardIterator>::value_type>::iterator, ForwardIterator>::value, ForwardIterator>*/
					{
           ...
           template<typename DVInputIterator, typename BinaryPredicate>
                ...
                const char * min_max )
            {
                ...
                default:
					 /*std::enable_if< std::is_base_of<typename device_vector<typename std::
                iterator_traits<DVInputIterator>::value_type>::iterator,DVInputIterator>::value, DVInputIterator >*/
					 {
