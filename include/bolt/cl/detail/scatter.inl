...
////////////////////////////////////////////////////////////////////
                          ...
                          const std::string& cl_code )
    {
        ...
        *********************************************************************************/
        ...
        kernels[boundsCheck].setArg( 4, result.base().getContainer().getBuffer() );
...
//    }
                                ...
                                const std::string& user_code)
    {
		...
		if(std::is_same< typename std::iterator_traits< InputIterator >::iterator_category ,
                                           std::random_access_iterator_tag
                                         >::value &&
		   std::is_same< typename std::iterator_traits< MapIterator >::iterator_category ,
                                           std::random_access_iterator_tag
                                         >::value)
		{
			      // Map the input iterator to a device_vector
		...
		else if(std::is_same< typename std::iterator_traits< InputIterator >::iterator_category ,
                                           std::random_access_iterator_tag
                                         >::value &&
		       (std::is_same< typename std::iterator_traits< MapIterator >::iterator_category ,
                                           bolt::cl::device_vector_tag
                                         ...
                                         >::value))
		{
				  ...
				  device_vector< iType1 > dvInput1( first_pointer, sz, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, true, ctl );
				  auto device_iterator_first = bolt::cl::create_device_itr(
                                            typename bolt::cl::iterator_traits< InputIterator >::iterator_category( ), 
...
////////////////////////////////////////////////////////////////////
                                       ...
                                       const std::string& user_code )
    {
        ...
        if( runMode == bolt::cl::control::SerialCpu )
        {
           ...
           dblog->CodePathTaken(BOLTLOG::BOLT_SCATTER,BOLTLOG::BOLT_SERIAL_CPU,"::Scatter::SERIAL_CPU");
           #endif
		   serial::scatter(/*ctl,*/ first1, last1, map, result/*, user_code*/);
        }
        else if( runMode == bolt::cl::control::MultiCoreCpu )
        {
#if defined( ENABLE_TBB )
            {
                ...
                dblog->CodePathTaken(BOLTLOG::BOLT_SCATTER,BOLTLOG::BOLT_MULTICORE_CPU,"::Scatter::MULTICORE_CPU");
                #endif
                //btbb::scatter(/*ctl, */first1, last1, map, result/*, user_code*/);
            }
#else
            throw std::runtime_error( "The MultiCoreCpu version of scatter is not enabled to be built! \n" );
